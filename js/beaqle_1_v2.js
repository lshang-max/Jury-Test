...
async function uploadTextToFileOnGitHub(txt, fileName) {
    ...
    try {
        const response = await fetch(url, {
            method: 'POST',
            headers: {
                'Authorization': `token ${secrets.JURYTEST_TOKEN}`,
...
//        this.TestState.Runtime.forEach(function(element, index, array){array[index] = 0});
    ...
    ListeningTest.prototype.nextTest = function() {
        ...
        if (this.TestState.CurrentTest === this.TestState.TestSequence.length - 1) {
            $('#BtnNextTest').text('Finish').css({
                padding: '0.5em 1.2em',           // Matches padding
                fontSize: '12.5pt',                // Standard font size (approximately 16px)
                fontWeight: 'normal',           // Default font weight
                fontFamily: '"Trebuchet MS", Arial, Helvetica, sans-serif', // Default font family
                border: '1px solid #999999',    // Border to match the standard
                borderRadius: '3px',            // Match border radius
                backgroundColor: '#ffffff',     // Background color
                color: '#354556',                // Text color
    ...
    ListeningTest.prototype.prevTest = function() {
        ...
        if ($('#BtnNextTest').text() === 'Finish') {
            var prevButtonStyles = $('#BtnPrevTest').css(['padding', 'font-size', 'font-weight', 'font-family', 'border', 'border-radius', 'background-color', 'color']);
            $('#BtnNextTest').text('Next Test').css(prevButtonStyles);
        }
    }
...
function JuryTest(TestData) {
    ListeningTest.apply(this, arguments);
}
JuryTest.prototype = new ListeningTest();
JuryTest.prototype.constructor = JuryTest;
...
// ###################################################################
// read ratings from TestState object
JuryTest.prototype.readRatings = function (TestIdx) {
...
// ###################################################################
// save ratings to TestState object
JuryTest.prototype.saveRatings = function (TestIdx) {
...
JuryTest.prototype.createTestDOM = function (TestIdx) {
...
JuryTest.prototype.formatResults = function () {
